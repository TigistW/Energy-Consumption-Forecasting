...
# # ... same imports, model classes, data loading and scaling ...
...
#             def autoregressive_forecast(model, input_seq, steps):
#                 preds = []
#                 seq = input_seq.clone()
#                 with torch.no_grad():
...
#                         next_row[:, 0] = pred  # Replace target feature with prediction
#                         seq = torch.cat((seq[:, 1:, :], next_row.unsqueeze(1)), dim=1)
#                 return preds

#             # Forecast both models
#             lstm_preds_scaled = autoregressive_forecast(lstm_model, input_seq, future_steps)
#             gru_preds_scaled = autoregressive_forecast(gru_model, input_seq, future_steps)

#             # Inverse transform predictions (only target feature)
#             dummy_lstm = np.zeros((future_steps, len(required_features)))
#             dummy_lstm[:, 0] = lstm_preds_scaled
#             lstm_preds = scaler.inverse_transform(dummy_lstm)[:, 0]

#             dummy_gru = np.zeros((future_steps, len(required_features)))
#             dummy_gru[:, 0] = gru_preds_scaled
#             gru_preds = scaler.inverse_transform(dummy_gru)[:, 0]

#             # Plot both forecasts + history
#             st.subheader("📈 Forecast Plot: LSTM vs GRU")
#             fig, ax = plt.subplots(figsize=(12, 4))
#             ax.plot(range(len(df)), df['Global_active_power'], label="History", color="black")
...
#             st.subheader("📋 Forecasted Values Comparison")
